<template>
  <div class="bg-white rounded-lg shadow mb-8">
    <div class=" px-8 py-4">
      ...
      <div v-for="education in all" :key="education.id" class="mb-4">
        <div class="flex justify-between">
          <div>
            <div class="text-gray-700 text-lg font-bold">{{ education.institution }}</div>
            <div class="text-gray-400 text-sm mb-2">{{ education.year }}</div>
          </div>
          <svg
...
import { Education, AllEducation } from '../types/api'

export default {
  computed: {
    all () {
      return this.$store.state.education.all
    },
  },
  mounted () {
    this.fetchData()
  ...
  methods: {
    ...
    async fetchData (): Promise<void> {
      this.setEducation(
        (await this.$axios.get('education')).data as AllEducation,
      )
    },
    async deleteEducation (education: Education): Promise<void> {
      ...
      await this.$axios.delete('education/' + education.id)
